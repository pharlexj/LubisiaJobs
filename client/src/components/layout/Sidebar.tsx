...
export default function Sidebar({ userRole }: SidebarProps) {
	...
	// useAuth provides the authenticated user and applicantProfile (if present)
	...
	const boardNavItems = [
		...
		},
		{
			...
			children: [
				...
				},
				{
					...
					icon: Users,
					label: "Hiring",
					description: "Award jobs to candidates",
				},
				{
					href: "/board/reports",
					icon: BarChart3,
		...
		},
		{
			...
			children: [
				{
					href: "/rms/board-committee",
					icon: LayoutDashboard,
					label: "Dashboard",
					description: "Committee review & documents",
				},
			],
		},
	];
	...
	const accountNavItems = [
		{
			...
			icon: LayoutDashboard,
			label: "Dashboard",
			description: "Accountant overview",
		},
		{
			id: "transactions",
			icon: DollarSign,
			...
			children: [
				...
				},
				{
					href: "/accountant/mir",
					icon: BookOpen,
					label: "Master Imprest",
					description: "MIR management",
				},
			],
		},
		{
	...
	const recordsOfficerNavItems = [
		{
			href: "/rms/records-officer",
			icon: LayoutDashboard,
			label: "Dashboard",
			description: "Doc. registry & overview",
		},
	];

	const boardSecretaryNavItems = [
		{
			href: "/rms/board-secretary",
			icon: LayoutDashboard,
			label: "Dashboard",
			description: "Doc. review & overview",
		},
	];

	const boardChairNavItems = [
		{
			href: "/rms/board-chair",
			icon: LayoutDashboard,
			label: "Dashboard",
			description: "Doc. review & remarks",
		},
	];

	const chiefOfficerNavItems = [
		{
			href: "/rms/chief-officer",
			icon: LayoutDashboard,
			label: "Dashboard",
			description: "Decision oversight & review",
		},
	];
	...
	const hrNavItems = [
		{
			...
			icon: LayoutDashboard,
			label: "Dashboard",
			description: "Agenda management & filing",
		},
	];
	...
	const getNavItems = () => {
		...
		switch (userRole) {
			case "applicant":
				baseItems = applicantNavItems;
				break;
			case "admin":
				baseItems = adminNavItems;
				break;
			case "board":
				baseItems = boardNavItems;
				break;
			case "accountant":
				baseItems = accountNavItems;
				break;
			case "a.i.e Holder":
				baseItems = aieNavItems;
				break;
			case "recordsOfficer":
				baseItems = recordsOfficerNavItems;
				break;
			case "boardSecretary":
				baseItems = boardSecretaryNavItems;
				break;
			case "boardChair":
				baseItems = boardChairNavItems;
				break;
			case "chiefOfficer":
				baseItems = chiefOfficerNavItems;
				break;
			case "boardCommittee":
				baseItems = boardCommitteeNavItems;
				break;
			case "HR":
				baseItems = hrNavItems;
				break;
			default:
				baseItems = [];
		}

		// Inject DIALs group when applicable
		return navItemsWithDials(baseItems);
	};

	// --- DIALs integration ---
	// Show DIALs to applicants who are employees (have an employment number) and
  // to staff roles (officer/reviewer/auditor/admin).| "applicant"
  
	const isApplicantEmployee =
		user?.role === "applicant" && (applicantProfile?.isEmployee);
	const isDialsStaff = [
		"admin",
		"boardCommittee",
		"auditor",
	...
	// DIALs navigation group (children built based on role)
	const dialsChildren: any[] = [];
	// Applicant-facing links (only for employees)
	dialsChildren.push({
		href: "/dials/dashboard",
		icon: FileText,
		label: "My Declarations",
		description: "View your declarations",
	});
	dialsChildren.push({
		href: "/dial/new",
		icon: ClipboardList,
	...
	// Staff links (only visible to reviewers/auditors/officers/admin)
	if (isDialsStaff) {
		dialsChildren.push({
			href: "/dial/review",
			icon: CheckCircle2,
			label: "Review Queue",
			description: "Submitted declarations",
		});
		dialsChildren.push({
			href: "/dial/declarations",
			icon: FileText,
			label: "All Declarations",
			description: "Browse declarations",
		});
		dialsChildren.push({
			href: "/dial/audit",
			icon: History,
		...
		dialsChildren.push({
			...
			icon: Settings,
			label: "DIAL Settings",
			description: "Manage DIAL settings",
		});
	}
	...
	const dialsGroup = {
		...
		label: "DIALs",
		icon: FileText,
		description: "Wealth Declarations",
		children: dialsChildren,
	};

	// If user should see DIALs, inject the group at the top of the nav list
	const navItemsWithDials = (baseItems: any[]) => {
		if (isApplicantEmployee || isDialsStaff) {
			// place DIALs at the start
			return [dialsGroup, ...baseItems];
		}
		return baseItems;
	};
				...
				<div className="relative p-4 border-b border-gray-200 flex items-center space-x-3">
					...
					{isOpen && (
						<div className="flex-1 min-w-0">
							...
							<Badge className={`${getRoleColor()} mt-1`}>
								{getRoleLabel()}
							</Badge>
						</div>
					)}
					...
					<TooltipProvider delayDuration={200}>
						{getNavItems().map((item: any) => {
							...
							const isGroupExpanded =
								item.id && expandedGroups.includes(item.id);

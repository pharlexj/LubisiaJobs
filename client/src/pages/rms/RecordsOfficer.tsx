...
export default function RecordsOfficer() {
	...
	const [dispatchAction, setDispatchAction] = useState<"dispatch" | "file">(
		"dispatch"
	);
	const [selectedDocument, setSelectedDocument] = useState<any>(null);
	const [documentFile, setDocumentFile] = useState<File | null>(null);
	const [forwardFile, setForwardFile] = useState<File | null>(null);
	const [forwardOurRef, setForwardOurRef] = useState<string>("");
	...
	const { data: documents, isLoading } = useQuery({
		...
		queryFn: async () => {
			const response = await apiRequest(
				"GET",
				"/api/rms/documents?includeDetails=true"
			);
	...
	const forwardDocumentMutation = useMutation({
		mutationFn: async ({ id, data }: { id: number; data: any }) => {
			// Support FormData uploads (when attaching a file or updating reference)
			if (data instanceof FormData) {
				const res = await fetch(`/api/rms/documents/${id}/forward`, {
					method: "POST",
					body: data,
					credentials: "include",
				});
	...
	const dispatchDocumentMutation = useMutation({
		mutationFn: async ({
			...
			// Send outcome along with decisionSummary so backend can handle either dispatch or filing
			return await apiRequest("POST", `/api/rms/documents/${id}/dispatch`, {
				decisionSummary,
				outcome,
			});
		},
	...
	const handleForwardDocument = () => {
		...
		// If there's a file to attach or our ref changed, use FormData
		if (
			forwardFile ||
			forwardOurRef !== (selectedDocument?.referenceNumber || "")
		) {
			const form = new FormData();
			form.append("toHandler", "boardSecretary");
			form.append("toStatus", "forwarded_to_secretary");
			form.append(
				"notes",
				`Forwarded to Board Secretary for review - ${
					selectedDocument?.subject || ""
				}`
			);
			if (forwardFile) form.append("document", forwardFile);
			if (forwardOurRef) form.append("referenceNumber", forwardOurRef);
			forwardDocumentMutation.mutate({ id: selectedDocument.id, data: form });
		} else {
			forwardDocumentMutation.mutate({
				...
				data: {
					...
					notes: `Forwarded to Board Secretary for review - ${
						selectedDocument?.subject || ""
					}`,
				},
			});
	...
	useEffect(() => {
		// When opening the dispatch dialog, set sensible default action
		if (showDispatchDialog && selectedDocument) {
			setDispatchAction(
		...
		// When opening forward dialog, prefill our ref and reset forward file
		if (showForwardDialog && selectedDocument) {
			setForwardOurRef(selectedDocument?.referenceNumber || "");
			setForwardFile(null);
		}
	}, [showDispatchDialog, selectedDocument, showForwardDialog]);
	...
	const getStatusBadge = (status: string) => {
		const statusConfig: Record<string, { label: string; className: string }> = {
			...
			sent_to_committee: {
				label: "At the Committee",
				className: "bg-pink-500",
			},
			returned_to_hr_from_committee: {
				label: "Returned from Committee",
				className: "bg-pink-600",
			},
			returned_to_secretary_from_hr: {
				label: "Returned to Secretary (from HR)",
			...
			sent_to_records_to_file: { label: "File away", className: "bg-teal-700" },
		...
		return (
			<Badge
				className={`${config.className} text-white px-3 py-1 rounded-full text-xs font-medium shadow-sm`}
			>
				{config.label}
			</Badge>
		);
	...
	const getPriorityBadge = (priority: string) => {
		...
		return (
			<Badge className={config.className} data-testid={`priority-${priority}`}>
						...
						<div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
							<div>
								...
								<p className="text-gray-600 mt-1">
									Document intake, tracking, and dispatch
								</p>
							</div>
							<Dialog
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p
												...
												data-testid="stat-received"
											>
												{(stats as any)?.received || 0}
											</p>
										</div>
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p
												...
												data-testid="stat-dispatched"
											>
												{(stats as any)?.dispatched || 0}
											</p>
										</div>
													...
													>
														...
														<td className="p-3">
															<div className="flex gap-2">
																...
																{doc.status === "sent_to_records_to_file" && (
																	<Button
																		size="sm"
																		className="bg-green-600 hover:bg-green-700 text-white"
																		onClick={() => {
			...
			<Dialog open={showForwardDialog} onOpenChange={setShowForwardDialog}>
				<DialogContent data-testid="dialog-forward-document">
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div>
								<Label htmlFor="our-ref">Our Ref</Label>
								<Input
									id="our-ref"
									value={forwardOurRef}
									onChange={(e) => setForwardOurRef(e.target.value)}
									data-testid="input-our-ref"
								/>
							</div>
							<div>
								<Label htmlFor="your-ref">Their Ref</Label>
								<Input
									id="your-ref"
									value={
										selectedDocument?.comments?.find(
											(c: any) => c.commentType === "external_ref"
										)?.comment || ""
									}
									readOnly
									data-testid="input-your-ref"
								/>
							</div>
						</div>

						<div>
							<Label htmlFor="forward-file">Attach File (optional)</Label>
							<Input
								id="forward-file"
								type="file"
								accept=".pdf,.doc,.docx"
								onChange={(e) => setForwardFile(e.target.files?.[0] || null)}
								data-testid="input-forward-file"
							/>
							{forwardFile && (
								<p className="text-sm text-gray-600 mt-2">
									Selected: {forwardFile.name}
								</p>
							)}
						</div>
					</div>
				...
				<DialogContent data-testid="dialog-dispatch-document">
					...
					<div className="space-y-4">
						...
						<div>
							...
							<Select
								...
								onValueChange={(value) =>
									setDispatchAction(value as "dispatch" | "file")
								}
							>
								<SelectTrigger
									id="dispatch-action"
									data-testid="select-dispatch-action"
								>
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
									<SelectItem value="dispatch">
										Dispatch to Initiator
									</SelectItem>
									<SelectItem value="file">
										File in Registry (File away)
									</SelectItem>
								</SelectContent>
							</Select>
						</div>
					...
					<DialogFooter>
						...
						>
							...
							{dispatchDocumentMutation.isPending
								? dispatchAction === "file"
									? "Processing..."
									: "Dispatching..."
								: dispatchAction === "file"

...
import AdminTabsList from "@/pages/admin/settings/AdminTabsList";
import DeptSection from "@/pages/admin/settings/DeptSection";
import NoticesSection from "@/pages/admin/settings/NoticesSection";
import BoardMembersList from "@/pages/admin/settings/BoardMembersList";
import GalleryList from "@/pages/admin/settings/GalleryList";
import CarouselList from "@/pages/admin/settings/CarouselList";
...
// Form schemas (some were accidentally removed previously)
...
const countySchema = z.object({
	name: z.string().min(2, "County name is required"),
});
...
const studyAreaSchema = z.object({
	name: z.string().min(2, "Study area name is required"),
});

const specializationSchema = z.object({
	name: z.string().min(2, "Specialization name is required"),
	studyAreaId: z.string().optional(),
});
...
export default function AdminSettings() {
	...
	const [galleryImageMode, setGalleryImageMode] = useState<"url" | "upload">(
		"url"
	);
	...
	const [carouselImageMode, setCarouselImageMode] = useState<
		"gradient" | "image"
	>("gradient");
	const [carouselImageFile, setCarouselImageFile] = useState<File | null>(null);
	const [carouselMobileImageFile, setCarouselMobileImageFile] =
		useState<File | null>(null);
	const [carouselImagePreview, setCarouselImagePreview] = useState<string>("");
	const [carouselMobilePreview, setCarouselMobilePreview] =
		useState<string>("");
	...
	const countyForm = useForm<CountyFormData>({
		resolver: zodResolver(countySchema),
	});
	const constituencyForm = useForm<ConstituencyFormData>({
		resolver: zodResolver(constituencySchema),
	});
	const wardForm = useForm<WardFormData>({ resolver: zodResolver(wardSchema) });
	const studyAreaForm = useForm<StudyAreaFormData>({
		resolver: zodResolver(studyAreaSchema),
	});
	const specializationForm = useForm<SpecializationFormData>({
		resolver: zodResolver(specializationSchema),
	});
	const jobGroupForm = useForm<JobGroupFormData>({
		resolver: zodResolver(jobGroupSchema),
	});
	const awardForm = useForm<AwardFormData>({
		resolver: zodResolver(awardSchema),
	});
	const ethnicityForm = useForm<EthnicityFormData>({
	...
	const roleAssignmentForm = useForm<RoleAssignmentFormData>({
		resolver: zodResolver(roleAssignmentSchema),
	});
	const aboutConfigForm = useForm<AboutConfigFormData>({
		resolver: zodResolver(aboutConfigSchema),
	});
	const galleryItemForm = useForm<GalleryItemFormData>({
		resolver: zodResolver(galleryItemSchema),
	});
	const boardMemberForm = useForm<BoardMemberFormData>({
	...
	const createMutation = useMutation({
		...
		onSuccess: (_, variables) => {
			...
			else if (variables.endpoint.includes("constituencies"))
				constituencyForm.reset();
			else if (variables.endpoint.includes("wards")) wardForm.reset();
			else if (variables.endpoint.includes("study-areas"))
				studyAreaForm.reset();
			else if (variables.endpoint.includes("specializations"))
			...
			else if (variables.endpoint.includes("system-config"))
				aboutConfigForm.reset();
			else if (variables.endpoint.includes("gallery")) galleryItemForm.reset();
			else if (variables.endpoint.includes("board-members"))
	...
	const tabs = [
		...
		},
		{
			...
			label: "Employees",
			icon: UploadCloud,
			description: "Upload Employees Data",
		},
		{
			id: "dept",
			label: "Department",
		...
		},
		{
			...
			label: "Documents",
			icon: FileUp,
			description: "Upload Documents in PDF",
		},
	];
		...
		createMutation.mutate(
			{
				...
				data: {
					...data,
					countyId: data.countyId ? parseInt(data.countyId, 10) : undefined,
				},
			},
			{
				onSuccess: () => {
		...
		createMutation.mutate({
			...
			data: {
				...
				constituencyId: data.constituencyId
					? parseInt(data.constituencyId, 10)
					: undefined,
			},
		});
	};
	...
	const handleCreateSpecialization = (data: SpecializationFormData) => {
		const studyAreaId = data.studyAreaId
			? parseInt(data.studyAreaId, 10)
			: undefined;
	...
	const handleCreateBoardMember = async (data: BoardMemberFormData) => {
		try {
			...
			if (boardPhotoMode === "upload" && boardPhotoFile) {
				...
				const uploadResult = await boardPhotoUpload.uploadFile(
					boardPhotoFile,
					"board-member-photo",
					{
				...
				photoUrl =
					uploadResult.fileUrl || uploadResult.url || uploadResult.path || "";
			}
			...
			if (editingBoardMember) {
				...
				// Create new member
				createMutation.mutate({
					endpoint: "/api/admin/board-members",
					data: processedData,
				});
			}
	...
	const handleCreateGalleryItem = async (data: GalleryItemFormData) => {
		try {
			...
			if (galleryImageMode === "upload" && uploadedImageFile) {
				...
				// Upload the file first
				const uploadResult = await galleryFileUpload.uploadFile(
					uploadedImageFile,
					"gallery-image",
					{
						title: data.title,
			...
			toast({
				...
				description:
					error.message ||
					`Failed to ${editingItem ? "update" : "create"} gallery item`,
				variant: "destructive",
			});
		}
	};
	...
	const filteredWards = selectedConstituency
		? wards.filter(
				(w: any) => w.constituencyId === parseInt(selectedConstituency)
		  )
		: [];

	const filteredSpecializations = selectedStudyArea
		? specializations.filter(
				(s: any) => s.studyAreaId === parseInt(selectedStudyArea)
		  )
		: [];
	...
	const handleCreateCarouselSlide = async (data: CarouselSlideFormDataExt) => {
		try {
			...
			// Optional fields
			processedData.accentColor =
				(data as Record<string, any>).accentColor || "";
			...
			processedData.displayOrder =
				typeof (data as Record<string, any>).displayOrder === "number"
					? (data as Record<string, any>).displayOrder
					: 0;
			...
			if (carouselImageMode === "image" && carouselImageFile) {
				...
				const uploadResult = await carouselImageUpload.uploadFile(
					carouselImageFile
				);
				if (uploadResult?.url) {
			...
			if (carouselMobileImageFile) {
				const mobileUploadResult = await carouselImageUpload.uploadFile(
					carouselMobileImageFile
				);
			...
			if (editingCarouselSlide) {
				...
				const response = await apiRequest(
					"PUT",
					`/api/admin/carousel-slides/${editingCarouselSlide.id}`,
					payload
				);
				toast({
					title: "Success",
				...
				// Create new slide
				createMutation.mutate({
					endpoint: "/api/admin/carousel-slides",
					data: payload,
				});
			}
						...
						>
							...
							<TabsContent value="dept">
								<DeptSection
									...
									onDelete={(id: number) =>
										handleDeleteItem(id, "/api/admin/dept", "department")
									}
									deptForm={deptForm}
								/>
							</TabsContent>
							...
							<TabsContent value="notices">
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										...
										<Dialog
											open={isModalOpen && activeTab === "notices"}
										...
										>
											...
											<DialogContent>
												<DialogHeader>
													<DialogTitle>
														{editingItem && editingType === "notice"
															? "Edit Notice"
															: "Create New Notice"}
													</DialogTitle>
												</DialogHeader>
												<form
													onSubmit={noticeForm.handleSubmit(handleCreateNotice)}
													className="space-y-4"
												>
													<div>
													...
													<div className="grid grid-cols-2 gap-4">
														<div>
															...
															<Select
																...
																onValueChange={(value) =>
																	noticeForm.setValue("type", value)
																}
															>
														...
														<div>
															...
															<Select
																...
																onValueChange={(value) =>
																	noticeForm.setValue("priority", value)
																}
															>
							...
							<TabsContent value="geography">
								<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
										...
										<CardHeader className="flex flex-row items-center justify-between">
											...
											<Dialog>
												...
												<DialogContent>
													...
													<form
														onSubmit={countyForm.handleSubmit(
															handleCreateCounty
														)}
														className="space-y-4"
													>
													...
													counties.map((county: any) => (
														...
														>
															...
															<Button
																...
																variant="outline"
																onClick={() =>
																	setSelectedCounty(county.id.toString())
																}
															>
																Select
															</Button>
										...
										<CardHeader className="flex flex-row items-center justify-between">
											...
											<Dialog>
												...
												<DialogContent>
													...
													<form
														onSubmit={constituencyForm.handleSubmit(
															handleCreateConstituency
														)}
														className="space-y-4"
													>
														<div>
															<Label htmlFor="constituency-county">
																County *
															</Label>
															<Select
															...
															<input
																type="hidden"
																{...constituencyForm.register("countyId")}
																value={selectedCounty}
															/>
														...
														<div>
															<Label htmlFor="constituency-name">
																Constituency Name *
															</Label>
															<Input
														...
														<div className="flex justify-end space-x-2">
															<Button
																type="submit"
																data-testid="button-add-constituency"
															>
																Add Constituency
															</Button>
														</div>
										...
										<CardContent>
											<div className="space-y-2 max-h-64 overflow-y-auto">
												{(selectedCounty
													...
													<p className="text-gray-500 text-center py-4">
														{selectedCounty
															? "No constituencies in selected county"
									...
									<Card>
										<CardHeader className="flex flex-row items-center justify-between">
											...
											<Dialog>
												...
												<DialogContent>
													...
													<form
														onSubmit={wardForm.handleSubmit(handleCreateWard)}
														className="space-y-4"
													>
														<div>
															<Label htmlFor="ward-constituency">
																Constituency *
															</Label>
															<Select
															...
															<input
																type="hidden"
																{...wardForm.register("constituencyId")}
																value={selectedConstituency}
															/>
										...
										<CardContent>
											<div className="space-y-2 max-h-64 overflow-y-auto">
												{(selectedConstituency ? filteredWards : wards)
													.length === 0 ? (
													<p className="text-gray-500 text-center py-4">
														{selectedConstituency
															? "No wards in selected constituency"
															: "Select a constituency first"}
													</p>
												) : (
													(selectedConstituency ? filteredWards : wards).map(
														(ward: any) => (
										...
										<CardHeader className="flex flex-row items-center justify-between">
											...
											<Dialog>
												...
												<DialogContent>
													...
													<form
														onSubmit={studyAreaForm.handleSubmit(
															handleCreateStudyArea
														)}
														className="space-y-4"
													>
														<div>
															<Label htmlFor="study-area-name">
																Study Area Name
															</Label>
															<Input
																id="study-area-name"
										...
										<CardHeader className="flex flex-row items-center justify-between">
											...
											<Dialog>
												...
												<DialogContent>
													...
													<form
														onSubmit={specializationForm.handleSubmit(
															handleCreateSpecialization
														)}
														className="space-y-4"
													>
														<div>
															<Label htmlFor="specialization-study-area">
																Study Area
															</Label>
															<Select
										...
										<CardContent>
											<div className="space-y-2 max-h-64 overflow-y-auto">
												{(selectedStudyArea
													...
													<p className="text-gray-500 text-center py-4">
														{selectedStudyArea
															? "No specializations in selected study area"
								...
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										<div>
											...
											<p className="text-sm text-gray-600 mt-1">
												Manage job groups and classifications
											</p>
										</div>
										<Dialog>
											<DialogTrigger asChild>
											...
											<DialogContent>
												...
												<form
													onSubmit={jobGroupForm.handleSubmit(
														handleCreateJobGroup
													)}
													className="space-y-4"
												>
													<div>
														<Label htmlFor="job-group-name">
															Job Group Name
														</Label>
														<Input
															id="job-group-name"
													...
													<div>
														<Label htmlFor="job-group-description">
										...
										<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
											{jobGroups.length === 0 ? (
												<div className="col-span-3 text-center py-8 text-gray-500">
													...
													<p>
														No job groups yet. Click "Add Job Group" to get
														started.
													</p>
												</div>
											) : (
									...
									<CardHeader className="flex flex-row items-center justify-between">
										...
										<Dialog>
											...
											<DialogContent>
												...
												<form
													onSubmit={awardForm.handleSubmit(handleCreateAward)}
													className="space-y-4"
												>
													<div>
								...
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										<div>
											...
											<p className="text-sm text-gray-600 mt-1">
												Manage ethnicity data and user roles
											</p>
										</div>
										<Dialog>
											<DialogTrigger asChild>
											...
											<DialogContent>
												...
												<form
													onSubmit={ethnicityForm.handleSubmit(
														handleCreateEthnicity
													)}
													className="space-y-4"
												>
													<div>
														<Label htmlFor="ethnicity-name">
															Ethnicity Name
														</Label>
														<Input
															id="ethnicity-name"
							...
							<TabsContent value="faqs">
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										...
										<Dialog
											open={isModalOpen && activeTab === "faqs"}
											onOpenChange={setIsModalOpen}
										>
											<DialogTrigger asChild>
											...
											<DialogContent className="max-w-2xl">
												<DialogHeader>
													<DialogTitle>
														{editingItem && editingType === "faq"
															? "Edit FAQ"
															: "Add New FAQ"}
													</DialogTitle>
												</DialogHeader>
												<form
													onSubmit={faqForm.handleSubmit(handleCreateFaq)}
													className="space-y-4"
												>
													<div>
														...
														<Select
															...
															onValueChange={(value) =>
																faqForm.setValue("category", value)
															}
														>
							...
							<TabsContent value="roles">
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										...
										<Dialog
											open={isModalOpen && activeTab === "roles"}
											onOpenChange={setIsModalOpen}
										>
											<DialogTrigger asChild>
											...
											<DialogContent>
												<DialogHeader>
													<DialogTitle>
														{editingItem && editingType === "role"
															? "Change User Role"
															: "Assign Role to User"}
													</DialogTitle>
												</DialogHeader>
												<form
													onSubmit={roleAssignmentForm.handleSubmit(
														handleCreateRoleAssignment
													)}
													className="space-y-4"
												>
													<div>
														...
														<div className="space-y-2">
															...
															<Select
																...
																onValueChange={(value) =>
																	roleAssignmentForm.setValue("userId", value)
																}
													...
													<div>
														...
														<Select
															...
															onValueChange={(value) =>
																roleAssignmentForm.setValue("role", value)
															}
														>
									...
									<CardHeader className="flex flex-row items-center justify-between">
										...
										<Dialog>
											...
											<DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
												...
												<form
													onSubmit={boardMemberForm.handleSubmit(
												...
												>
													...
													<div className="space-y-4">
														...
														<div className="flex rounded-md border border-gray-200 p-1">
															<Button
																type="button"
																variant={
																	boardPhotoMode === "url" ? "default" : "ghost"
																}
																size="sm"
																className="flex-1"
																onClick={() => {
																...
																<div
																	...
																	onClick={() => {
																		...
																		input.onchange = (e) => {
																			...
																			if (file)
																				handleBoardPhotoFileSelect(file);
																		};
																		input.click();
																	}}
																		...
																		<div className="space-y-2">
																			...
																			<div>
																				...
																				<p className="text-xs text-gray-500">
																					or click to browse (JPEG, PNG up to
																					5MB)
																				</p>
																			</div>
																		</div>
													...
													<div>
														...
														<Input
															id="board-order"
															type="number"
															{...boardMemberForm.register("order", {
																valueAsNumber: true,
															})}
															placeholder="0"
														/>
										...
										<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
											{(boardMembers as any[]).length === 0 ? (
												<div className="col-span-3 text-center py-8 text-gray-500">
													<Users className="w-16 h-16 mx-auto mb-4 text-gray-300" />
													<p>
														No board members yet. Click "Add Board Member" to
														get started.
													</p>
												</div>
											) : (
													...
													<Card key={member.id}>
														<CardContent className="p-4 text-center relative">
															...
															<h4
																...
																data-testid={`text-board-member-name-${member.id}`}
															>
																{member.name}
															</h4>
															<p
																className="text-primary font-medium"
																data-testid={`text-board-member-position-${member.id}`}
															>
																{member.position}
															</p>
															{member.bio && (
																<p
																	...
																	data-testid={`text-board-member-bio-${member.id}`}
																>
																	{member.bio}
																</p>
															)}
															<Badge
																className="mt-2"
																variant="outline"
																data-testid={`badge-board-member-order-${member.id}`}
															>
																Order: {member.order || 0}
															</Badge>
														</CardContent>
								...
								<div className="space-y-6">
										...
										<CardHeader className="flex flex-row items-center justify-between">
											...
											<Dialog>
												...
												<DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
													<DialogHeader>
														<DialogTitle>
															{editingItem && editingType === "about-config"
																? "Edit About Page Configuration"
																: "Add About Page Configuration"}
														</DialogTitle>
													</DialogHeader>
													<form
														onSubmit={aboutConfigForm.handleSubmit(
															handleCreateAboutConfig
														)}
														className="space-y-4"
													>
														<div>
															...
															<Select
																onValueChange={(value) =>
																	aboutConfigForm.setValue("key", value)
																}
															>
																<SelectTrigger>
																...
																<SelectContent>
																	<SelectItem value="heroTitle">
																		Hero Title
																	</SelectItem>
																	<SelectItem value="heroDescription">
																		Hero Description
																	</SelectItem>
											...
											<div className="space-y-4">
												{Object.keys(aboutConfig).length === 0 ? (
													<div className="text-center py-8 text-gray-500">
														...
														<p>
															No about page configuration found. Click "Add
																	...
																	<div className="flex space-x-2 ml-4">
																		...
																		<Button
																			...
																			onClick={() =>
																				handleDeleteItem(
																					0,
																					`/api/admin/system-config?section=about&key=${encodeURIComponent(
																						key
																					)}`,
																					"configuration item"
																				)
									...
									<Card>
										<CardHeader>
											...
											<p className="text-sm text-gray-600 mt-1">
												Manage contact details displayed on the about page
											</p>
										</CardHeader>
										<CardContent>
											<div className="space-y-4">
								...
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										<div>
											...
											<p className="text-sm text-gray-600 mt-1">
												Manage gallery items and images
											</p>
										</div>
										<Dialog
											open={isModalOpen && activeTab === "gallery"}
											onOpenChange={setIsModalOpen}
										>
											<DialogTrigger asChild>
											...
											<DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
												...
												<form
													onSubmit={galleryItemForm.handleSubmit(
												...
												>
													...
													<div>
														...
														<Select
															onValueChange={(value) =>
																galleryItemForm.setValue("category", value)
															}
														>
															<SelectTrigger>
																<SelectValue placeholder="Select category" />
													...
													<div>
														<Label htmlFor="gallery-eventDate">
															Event Date
														</Label>
														<Input
									...
									<GalleryList
										...
										onDelete={(id: number) =>
											handleDeleteItem(id, "/api/admin/gallery", "gallery item")
										}
									/>
								</Card>
							</TabsContent>
								...
								<Card>
									<CardHeader>
										<div>
											...
											<p className="text-sm text-gray-600 mt-1">
												Manage system-wide settings including favicon
											</p>
										</div>
									</CardHeader>
									<CardContent>
									...
									<CardHeader className="flex flex-row items-center justify-between">
										...
										<Dialog>
											...
											<DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
												...
												<form
													onSubmit={carouselSlideForm.handleSubmit(
												...
												>
													<div>
														...
														<Input
															id="slide-title"
															{...carouselSlideForm.register("title")}
														/>
													</div>
													<div>
													...
													<div>
														...
														<Select
															onValueChange={(v) =>
																carouselSlideForm.setValue("iconName", v as any)
															}
													...
													{carouselImageMode === "gradient" ? (
														<div className="space-y-2">
															...
															<div className="flex gap-2 items-center">
																...
																<Select
																	onValueChange={(v) =>
																		carouselSlideForm.setValue("gradientDir", v)
																	}
																	defaultValue={
																		carouselSlideForm.watch("gradientDir") ||
																		"r"
																	}
																>
															...
															<Input
																...
																value={`from-[${
																	carouselSlideForm.watch("gradientStart") ||
																	"#3b82f6"
																}] to-[${
																	carouselSlideForm.watch("gradientEnd") ||
																	"#a21caf"
																}] bg-gradient-to-${
																	carouselSlideForm.watch("gradientDir") || "r"
																}`}
																readOnly
															/>
														...
														<>
															<div>
																...
																<Input
																	...
																	onChange={(e) =>
																		e.target.files?.[0] &&
																		setCarouselImageFile(e.target.files[0])
																	}
																/>
													...
													<div className="space-y-2">
														...
														<Input
															id="alt-text"
															{...carouselSlideForm.register("altText")}
														/>
														<Label htmlFor="link-href">Link (optional)</Label>
														<Input
															id="link-href"
															{...carouselSlideForm.register("linkHref")}
														/>
														<Label htmlFor="cta-label">CTA Label</Label>
														<Input
															id="cta-label"
															{...carouselSlideForm.register("ctaLabel")}
														/>
														<Label htmlFor="display-order">Order</Label>
														<Input
															id="display-order"
															type="number"
															{...carouselSlideForm.register("displayOrder", {
																valueAsNumber: true,
															})}
														/>
														<div className="flex items-center gap-2 mt-2">
															...
															<Input
																id="is-active"
																type="checkbox"
																{...carouselSlideForm.register("isActive")}
															/>
														</div>

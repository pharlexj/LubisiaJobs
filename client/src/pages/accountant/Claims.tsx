							...
							>
								...
								<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
									<div>
										...
										>
											...
											<SelectContent>
												<SelectItem value="0">Select Vote</SelectItem>
												{votes.map((vote) => (
									...
									<div>
										...
										>
											<SelectTrigger
												...
												className="w-40 px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
											>
												<SelectValue placeholder="Select Account" />
											</SelectTrigger>
											<SelectContent>
												<SelectItem value="0">Select Account</SelectItem>
									...
									<div className="md:col-span-2">
										...
										<Select
											...
											onValueChange={(val: string) =>
												setFormData({ ...formData, aieHolderId: val })
											}
										>
											<SelectTrigger
												...
												className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
											>
												<SelectValue placeholder="Select AIE Holder" />
											</SelectTrigger>
											<SelectContent>
												<SelectItem value="0">Select AIE Holder</SelectItem>
												{employees
													.filter(
														(e) =>
															e.designation &&
															e.designation.toLowerCase().includes("holder")
													)
													.map((emp) => (
										...
										{employees.map((emp) => (
											...
											>
												<input
													...
													onChange={() =>
														toggleEmployeeSelection(emp.id.toString())
													}
													className="rounded border-gray-300"
													data-testid={`checkbox-employee-${emp.id}`}
												/>
												<span className="text-sm">
													{emp.name} - {emp.personalNumber} - JG "{emp.jg}"
												</span>
											</label>
										))}
									</div>
									<p className="text-xs text-gray-500 mt-1">
										Selected: {formData.employeeIds.length} payee(s)
									</p>
								</div>
									...
									<div>
										...
										<input
											...
											onChange={(e) =>
												setFormData({
													...formData,
													busFare: parseFloat(e.target.value) || 0,
												})
											}
											className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
											placeholder="0.00"
									...
									<div>
										...
										<input
											...
											onChange={(e) =>
												setFormData({
													...formData,
													taxiFare: parseFloat(e.target.value) || 0,
												})
											}
											className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
											placeholder="0.00"
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
									<div className="md:col-span-2">
										...
										<input
											...
											onChange={(e) =>
												setFormData({
													...formData,
													reasonForPayment: e.target.value,
												})
											}
											className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
									...
									<div>
										...
										<Select value={formData.station} onValueChange={(val: string) =>
												setFormData({ ...formData, station: val })
											}>
											<SelectTrigger className="w-40">
												<SelectValue placeholder="All Status" />
											</SelectTrigger>
											<SelectContent>
												<SelectItem value="all">All Station</SelectItem>
								...
								<div className="bg-blue-50 border-l-4 border-blue-600 p-3 rounded">
									<p className="text-sm font-semibold text-blue-900">
										Total Amount: Ksh.{" "}
										{(formData.busFare + formData.taxiFare).toLocaleString()}
									</p>
								</div>
												...
												claims.map((claim: any, idx: number) => (
													...
													>
														...
														<td className="px-3 py-2 text-xs font-semibold">
															Ksh.{" "}
															{parseFloat(claim.amounts || 0).toLocaleString()}
														</td>

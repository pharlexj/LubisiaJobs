...
export default function Payments() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const employee = employees.find(
			(e) => e.id === parseInt(formData.merchandise)
		);

		// Combine all particulars into a single description
		const particularsList = particulars
			.map((p) => p.description)
			.filter((d) => d)
			.join("; ");
							...
							>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
									<div className="md:col-span-1">
										<label className="block text-xs font-semibold text-gray-700 mb-1">
											Select Merchandise/Payee
										</label>
										<Select
											required
											value={formData.merchandise}
											onValueChange={(val: string) =>
												setFormData({ ...formData, merchandise: val })
											}
										>
											<SelectTrigger
											...
											<SelectContent>
												<SelectItem value="0">Select Merchandise</SelectItem>
												{employees.map((emp) => (
													<SelectItem key={emp.id} value={emp.id.toString()}>
										...
										<table className="w-full text-sm">
											<thead className="bg-gray-100 border-b">
												<tr>
													...
													<th className="px-2 py-2 text-left text-xs font-semibold text-gray-700 w-32">
														Amount (Ksh)
													</th>
											...
											<tfoot className="bg-gray-100 border-t">
												<tr>
													<td
														colSpan={2}
														className="px-2 py-2 text-xs font-semibold text-gray-700 text-right"
													>
														Total Amount:
													</td>
													<td
														className="px-2 py-2 text-xs font-bold text-blue-900"
														colSpan={2}
													>
												...
												payments.map((payment: any, idx: number) => (
													...
													>
														...
														<td className="px-3 py-2 text-xs">
															{new Date(
																payment.dated || payment.createdAt
															).toLocaleDateString()}
														</td>

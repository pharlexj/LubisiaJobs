...
export default function Vote() {
	...
	const deleteMutation = useMutation({
		mutationFn: async (id: number) => {
			const res = await fetch(`/api/accounting/vote-accounts/${id}`, {
				method: "DELETE",
			});
		...
		onSuccess: () =>
			queryClient.invalidateQueries({ queryKey: ["/api/accounting/votes"] }),
	});
	...
	const bulkMutation = useMutation({
		mutationFn: async (file: File) => {
			...
			const res = await fetch("/api/accounting/vote-accounts/bulk", {
				method: "POST",
				body: form,
			});
			if (!res.ok) {
													...
													{isLoading ? (
														...
														<TableRow>
															<TableCell
																...
																className="text-center py-8 text-gray-500"
															>
																No votes found. Add your first vote!
															</TableCell>
														</TableRow>
													) : (
														votes.map((vote: any, idx: number) => (
															<TableRow key={vote.id}>
																...
																<TableCell>{vote.votedItems}</TableCell>
																<TableCell>{vote.voteType || "-"}</TableCell>
																...
																<TableCell className="flex gap-2">
																	...
																	<Button
																		...
																		variant="destructive"
																		onClick={() => {
																			if (!confirm("Delete this vote account?"))
																				return;
																			deleteMutation.mutate(vote.id);
																		}}
																	>
							...
							<div className="col-span-12 lg:col-span-4">
								<div className="rounded-lg overflow-hidden shadow-sm">
									<div className="flex items-center justify-between px-4 py-3 bg-red-600 text-white">
										...
										<a
											href="/api/accounting/vote-accounts/template"
											className="inline-flex items-center gap-2 bg-white text-red-600 px-3 py-1 rounded-md text-sm"
										>
											<svg
									...
									<div className="bg-white p-4">
										<p className="text-sm text-gray-600">
											Download the template, fill it with vote data, then upload
											here.
										</p>
										<div className="mt-4">
										...
										{bulkResult && (
											<div className="mt-4 bg-gray-50 p-3 rounded">
												<div className="text-sm">
													Imported:{" "}
													{bulkResult.created || bulkResult.createdCount || 0} /{" "}
													{bulkResult.total || 0}
												</div>
												{bulkResult.errors && (
													<div className="text-xs text-red-600 mt-2">
														Errors: {JSON.stringify(bulkResult.errors)}
													</div>
												)}
											</div>
										)}
